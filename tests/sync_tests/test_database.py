...
def test_list_databases_with_pagination(
    client, databaselist_model_mock, mocker: MockFixture
):
    mock_get = mocker.patch("httpx.Client.get")
    ...
    response = client.list_databases(cont=10, limit=25)
...
def test_list_databases_not_found(client, mocker: MockFixture):
    mock_get = mocker.patch("httpx.Client.get")
...
def test_get_database_not_found(client, mocker: MockFixture):
    ...
    with pytest.raises(Exception) as exc:
        client.get_database(name=MOCK_DATABASE_NAME)
...
def test_create_database_with_dict(client, database_model_mock, mocker: MockFixture):
    ...
    response = client.create_database(body=body_dict)

    assert isinstance(response, Database)
    assert response.metadata.name == MOCK_DATABASE_NAME


def test_create_database_conflict(client, database_body, mocker: MockFixture):
...
def test_update_database_success(
    client, database_patch_body, database_model_mock, mocker: MockFixture
):
...
def test_update_database_with_dict(client, database_model_mock, mocker: MockFixture):
    ...
    response = client.update_database(name=MOCK_DATABASE_NAME, body=body_dict)

    assert isinstance(response, Database)
    assert response.metadata.name == MOCK_DATABASE_NAME


def test_update_database_not_found(client, database_patch_body, mocker: MockFixture):
    mock_put = mocker.patch("httpx.Client.put")
    ...
    with pytest.raises(Exception) as exc:
        client.update_database(name=MOCK_DATABASE_NAME, body=database_patch_body)
...
def test_delete_database_success(client, mocker: MockFixture):
    ...
    response = client.delete_database(name=MOCK_DATABASE_NAME)

    assert response is None


def test_delete_database_not_found(client, mocker: MockFixture):
    mock_delete = mocker.patch("httpx.Client.delete")
    ...
    with pytest.raises(Exception) as exc:
        client.delete_database(name=MOCK_DATABASE_NAME)

    assert str(exc.value) == "database not found"


def test_list_database_versions_success(client, mocker: MockFixture):
    mock_get = mocker.patch("httpx.Client.get")
    ...
    response = client.list_database_versions()
...
def test_list_backups_success(client, backuplist_model_mock, mocker: MockFixture):
    ...
    response = client.list_backups(database_name=MOCK_DATABASE_NAME)
...
def test_list_backups_with_pagination(
    client, backuplist_model_mock, mocker: MockFixture
):
    mock_get = mocker.patch("httpx.Client.get")
    ...
    response = client.list_backups(database_name=MOCK_DATABASE_NAME, cont=5, limit=20)
...
def test_list_backups_not_found(client, mocker: MockFixture):
    mock_get = mocker.patch("httpx.Client.get")
    ...
    with pytest.raises(Exception) as exc:
        client.list_backups(database_name=MOCK_DATABASE_NAME)
...
def test_get_backup_not_found(client, mocker: MockFixture):
    ...
    with pytest.raises(Exception) as exc:
        client.get_backup(database_name=MOCK_DATABASE_NAME, backup_id=MOCK_BACKUP_ID)
...
def test_delete_backup_not_found(client, mocker: MockFixture):
    mock_delete = mocker.patch("httpx.Client.delete")
    ...
    with pytest.raises(Exception) as exc:
        client.delete_backup(database_name=MOCK_DATABASE_NAME, backup_id=MOCK_BACKUP_ID)

    assert str(exc.value) == "backup not found"


def test_restore_backup_success(client, mocker: MockFixture):
    mock_post = mocker.patch("httpx.Client.post")
...
def test_restore_backup_conflict(client, mocker: MockFixture):
...
def test_database_with_standby_and_backup(client, database_model_mock, mocker: MockFixture):
...
def test_database_with_migration(client, database_model_mock, mocker: MockFixture):
