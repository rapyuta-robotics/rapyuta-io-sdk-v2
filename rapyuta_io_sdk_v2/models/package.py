...
class LivenessProbe(BaseModel):
    ...
    initialDelaySeconds: int | None = Field(default=1, ge=1)
    timeoutSeconds: int | None = Field(default=10, ge=10)
    periodSeconds: int | None = Field(default=1, ge=1)
    successThreshold: int | None = Field(default=3, ge=1)
    failureThreshold: int | None = Field(default=3, ge=1)
...
class EnvironmentSpec(BaseModel):
    ...
    exposed: bool | None = Field(default=False)
...
class RosEndpointSpec(BaseModel):
    type: str
    name: str
    compression: bool | None = Field(default=False)
